...
{
class MotorHardwareInterface : public ActuatorInterface
{
  CallbackReturn on_init(const hardware_interface::HardwareInfo & actuator_info) override
  {
    ...
    if (this->info_.joints.size() != 1)
    {
      return CallbackReturn::ERROR;
    }
    // can only control in position
    const auto & command_interfaces = this->info_.joints[0].command_interfaces;
    ...
    const auto & state_interfaces = this->info_.joints[0].state_interfaces;
    ...
    fprintf(stderr, "MotorHardwareInterface configured successfully.\n");
  ...
  std::vector<StateInterface> export_state_interfaces() override
  {
    ...
    const auto & joint_name = this->info_.joints[0].name;
